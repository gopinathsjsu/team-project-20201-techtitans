...
function RestaurantManagerRestaurantBookings() {
	...
	async function fetchReservationsByRestaurantID() {
		try {
			const response = await axios.get(
				`http://restaurant-api-alb-405497354.us-east-2.elb.amazonaws.com:5000/reservations/restaurant/${id}`
			);
			return response.data;
		} catch (error) {
			console.error(
	...
	async function fetchUserByUserID(userId) {
		...
		try {
			const response = await axios.get(
				`http://restaurant-api-alb-405497354.us-east-2.elb.amazonaws.com:5000/username/${userId}`
			);
