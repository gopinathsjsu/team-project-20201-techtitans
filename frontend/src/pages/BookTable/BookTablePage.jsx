      ...
      const filteredResults = allRestaurants
        .map((restaurant) => {
			...
			if (!hoursForDay || hoursForDay === "Closed") {
				return { ...restaurant, availableTimes: ["Restaurant is Closed"] };
			  }
      ...
      <div className="search-results">
        {searchResults.length === 0 ? (
          <div className="no-results">
            <h2>
              Oops! We don't have any restaurants available for the chosen
              slot.
            </h2>
            <h2>
              ...
              <div className="restaurant-content">
                ...
                <div className="available-times">
                  ...
                  <div className="time-slots">
                    ...
                    result.availableTimes[0] === "Restaurant is Closed" ? (
                      <p>Restaurant is Closed</p>
                    ) : (
                      <>
                        {(() => {
                          const selectedTime = convertTo12Hour(
                            getRoundedTime(searchCriteria.time)
                          );
                          const index =
                            result.availableTimes.indexOf(selectedTime);
                          if (index === -1) return null;

                          // Show a few slots near the searched time
                          const start = Math.max(0, index - 2);
                          const visibleSlots = result.availableTimes.slice(
                            start,
                            index + 1
                          );
                          ...
                          return visibleSlots.map((time, idx) => (
                            <button
                              key={idx}
                              onClick={() =>
                                handleTimeSlotClick(result.id, time)
                              }
                        ...
                        {result.availableTimes.length > 5}
                      </>
                    )}
                  </div>
                </div>
